...
SELECT 
	...
	MoreInfo = 'EXEC dbo.sp_BlitzCache @DatabaseName=''StackOverflow'', @OnlySqlHandles=''' + CONVERT(VARCHAR(MAX), qs.[sql_handle], 1) + ''''
INTO #sql_handles
...
WHERE 1=1 
	...
	AND DB_NAME(st.[dbid]) = 'StackOverflow'
	AND st.[text] NOT LIKE '%dm_exec_cached_plans%'
	AND st.[text] NOT LIKE '%##BlitzCacheProcs%'

IF 1 = (SELECT COUNT(*) FROM #sql_handles)
BEGIN
	DECLARE @SqlHandle AS VARCHAR(MAX)
	SELECT @SqlHandle = CONVERT(VARCHAR(MAX), sh.SqlHandle, 1) FROM #sql_handles AS sh
	SELECT [Executing...] = 'EXEC dbo.sp_BlitzCache @DatabaseName=''StackOverflow'', @OnlySqlHandles=''' + @SqlHandle + ''''
	EXEC dbo.sp_BlitzCache @DatabaseName='StackOverflow', @OnlySqlHandles=@SqlHandle
END
ELSE
BEGIN
